...
@Component({
  ...
  templateUrl: './view-documento-programa.component.html',
...
export class ViewDocumentoProgramaComponent implements OnInit {
  ...
  loadData(): void {
    ...
    this.inscripcionService.get('soporte_documento_programa?query=InscripcionId.Id:' +
      this.inscripcion_id + ',DocumentoProgramaId.ProgramaId:' + this.programa_id + ',DocumentoProgramaId.TipoInscripcionId:' + this.tipoInscripcion_id + ',DocumentoProgramaId.PeriodoId:' + parseInt(sessionStorage.getItem('IdPeriodo')!, 10) + ',DocumentoProgramaId.Activo:true,Activo:true&limit=0').subscribe(
        // (response: any[]) => {
        (response: any) => {
